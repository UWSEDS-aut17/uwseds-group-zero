import sys
import os
from . import object_detection_main as od
...
def video_open(cap, flag, modelPath, labelMapPath, savepath, test):
    ...
    save = False
    if savepath is not None and not \
        savepath == "" and \
            os.path.isdir(savepath) is True:
        save = True
    new_video_Process = od.objectDetection(modelPath, labelMapPath)
    while (cap.isOpened()):
        # sent to object detection
        # stop streaming and
        # release the camera if window close or "q" is pressed
